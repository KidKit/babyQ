...
- (UIBarButtonItem *)refreshBarButtonItem {
    ...
    if ([self isSystemIOS7] && ![self isIPhone]){
        refreshBarButtonItem.imageInsets = UIEdgeInsetsMake(0.0, 0, 0, 0);
    }
...
- (UIBarButtonItem *)stopBarButtonItem {
    ...
    if ([self isSystemIOS7] && ![self isIPhone]){
        stopBarButtonItem.imageInsets = UIEdgeInsetsMake(0.0, 0, 0, 0);
    }
...
- (UIBarButtonItem *)actionBarButtonItem {
    ...
    if ([self isSystemIOS7] && ![self isIPhone]){
        actionBarButtonItem.imageInsets = UIEdgeInsetsMake(0.0, 0, 0, 0);
    }
...
- (void)doneButtonClicked:(id)sender {
    if ([self.delegate respondsToSelector:@selector(svWebViewControllerWillDismiss:)]) {
        [self.delegate svWebViewControllerWillDismiss:self];
    }
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 60000
    [self dismissModalViewControllerAnimated:YES];
    if ([self.delegate respondsToSelector:@selector(svWebViewControllerDidDismiss:)]){
        [self.delegate svWebViewControllerDidDismiss:self];
    }
#else
